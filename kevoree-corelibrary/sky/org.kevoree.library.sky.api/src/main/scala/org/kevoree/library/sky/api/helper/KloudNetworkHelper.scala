    ...
    model.getNodes.foreach {
      node =>
        if (node.getDictionary.isDefined) {
          node.getDictionary.get.getValues.filter(value => value.getAttribute.getFragmentDependant && (value.getAttribute.getName == "port" || value.getAttribute.getName.startsWith("port") || value.getAttribute.getName.endsWith("port"))).foreach(
        ...
        node.getComponents.foreach {
          component =>
            if (component.getDictionary.isDefined) {
              component.getDictionary.get.getValues.filter(value => value.getAttribute.getFragmentDependant && (value.getAttribute.getName == "port" || value.getAttribute.getName.startsWith("port") || value.getAttribute.getName.endsWith("port"))).foreach(
                value => ports = ports + Integer.parseInt(value.getValue)
              )
            }
        }
    ...
    model.getGroups.foreach {
      group =>
        if (group.getDictionary.isDefined) {
          group.getDictionary.get.getValues.filter(value => value.getAttribute.getFragmentDependant && (value.getAttribute.getName == "port" || value.getAttribute.getName.startsWith("port") || value.getAttribute.getName.endsWith("port"))).foreach(
            value => ports = ports + Integer.parseInt(value.getValue)
          )
        }
    }
    model.getHubs.foreach {
      channel =>
        if (channel.getDictionary.isDefined) {
          channel.getDictionary.get.getValues.filter(value => value.getAttribute.getFragmentDependant && (value.getAttribute.getName == "port" || value.getAttribute.getName.startsWith("port") || value.getAttribute.getName.endsWith("port"))).foreach(
            value => ports = ports + Integer.parseInt(value.getValue)
          )
        }
    }
