 ...
 * one and only one node to act as a server (so with a given port in this group's
 * attributes) You can only trigger PUSH events on the master server or the
 * event will be lost into the wild. Once you've pushed a model on the master
 * server, this node fragment will handle model broadcasting on each connected
 * node (so if a node hasn't established a connection to this server he will not
 * get notified)
 * 
 * @author Leiko
 * 
...
public class WebSocketGroupMasterServer extends AbstractGroupType {
	...
	public void start() throws MultipleMasterServerException {
		...
		if (masterPort != null) {
			...
			for (ContainerNode subNode : group.getSubNodes()) {
				...
				if (groupOption != null) {
					Option<String> portOption = KevoreePropertyHelper.getProperty(groupOption, "port", true, subNode.getName());
					if (portOption.isDefined() && !subNode.getName().equals(getNodeName())) {
						// this node is not "me" and has already a defined port.. sounds like we have multiple master server
						throw new MultipleMasterServerException("You are not supposed to give multiple master server with this group. Just give a port to one and only one node in this group.");
					}
				}
			}
			...
			logger.debug("Master WebSocket server started on "+server.getUri().toString());
		}
	...
	protected String getMasterServerNodeName() throws NoMasterServerFoundException {
		Group group = getModelElement();
		ContainerRoot model = getModelService().getLastModel();
		for (ContainerNode subNode : group.getSubNodes()) {
			Group groupOption = model.findByPath("groups[" + getName() + "]", Group.class);
			if (groupOption != null) {
				Option<String> portOption = KevoreePropertyHelper.getProperty(groupOption, "port", true, subNode.getName());
				if (portOption.isDefined()) return subNode.getName();
			}
		}
	...
	public void push(ContainerRoot model, String targetNodeName) throws Exception {
		if (targetNodeName.equals(getMasterServerNodeName())) {
			...
			throw new NotAMasterServerException("Push request can only be made on master server node.");			
		}
	}
	
	private void pushModel(byte[] data, String ip, int port) throws Exception {
		logger.debug("Sending model via webSocket client to master server "+"ws://" + ip + ":" + port + "/");
		...
		for (ContainerNode subNode : group.getSubNodes()) {
			...
			if (groupOption != null) {
				...
				if (portOption.isDefined()) {
					...
					for (String ip : ips) {
						// for each ip in node network properties add an entry to the map 
						map.put(ip, port);
					}
				}
			}
	...
	private BaseWebSocketHandler handler = new BaseWebSocketHandler() {
		...
		public void onMessage(WebSocketConnection connection, byte[] msg) throws Throwable {
			switch (msg[0]) {					
				...
				default:
					logger.debug("Receiving "+msg[0]+" as byte[0]: do NOT know this message. Known bytes are "+PULL+", "+PUSH);
					break;
			}
		}
		...
		public void onOpen(WebSocketConnection connection) throws Exception {
			...
			logger.debug("OPEN: New client added to active connections: "+connection.httpRequest().remoteAddress());
		}
		
		@Override
		public void onClose(WebSocketConnection connection) throws Exception {
			clients.remove(connection);
			logger.debug("CLOSE: Client "+connection.httpRequest().remoteAddress()+" closed connection with server. Removed from active connections.");
		}
	};
				...
				if (serverEntries.isEmpty()) {
					...
					for (Entry<String, Integer> entry : serverEntries.entrySet()) {
						...
						try {
							...
							logger.error("Unable to register client on master server "+ip+":"+port+", maybe it's not the right ip:port");
						}
					}
				}
			}
