...
@GroupType
public class WebSocketGroupMasterServerOld extends AbstractGroupType implements DeployUnitResolver {
		...
		if (port != null) {
			...
			if (serverEntries.isEmpty()) {
                ...
                try {
                    ...
                    logger.warn("Unable to parse property \"reconnectDelay\".. using default value {}", DEFAULT_RECONNECT_DELAY);
                }
                ...
                if (mvnRepo) {
                    // there is a maven repo hosted on master server
                    // so we need to create some objects for that in clients
		...
		if (targetNodeName.equals(getMasterServerNodeName())) {
			...
			for (Entry<String, Integer> entry : serverEntries.entrySet()) {
				...
				if (client.connectBlocking()) {
                    ...
                    logger.warn("Unable to connect to master server on {}", client.getURI());
                }
			}
		} else {
	...
	private void pushModel(byte[] data, String ip, int port) throws Exception {
        ...
        if (client.connectBlocking()) {
            ...
            logger.warn("Connection to master server {} impossible", client.getURI());
        }
	}
...
//        }
	...
	public void stop() {
		if (client != null) {
            ...
            wsClientHandler.stopAllTasks(); // clear pending connection tasks
    ...
    public boolean preUpdate(ContainerRoot currentModel, ContainerRoot proposedModel) {
        ...
        if (client != null && mvnRepo) {
            ...
            for (Entry<String, Integer> entry: getMasterServerEntries().entrySet()) {
	...
	protected void updateLocalModel(final ContainerRoot model) {
		...
		new Thread() {
			public void run() {
				try {
					getModelService().unregisterModelListener(WebSocketGroupMasterServerOld.this);
					getModelService().atomicUpdateModel(model);
					getModelService().registerModelListener(WebSocketGroupMasterServerOld.this);
				} catch (Exception e) {
					logger.error("", e);
				}
